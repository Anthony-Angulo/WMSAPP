...
export class AbarrotesBatchPage implements OnInit {
  ...
  ngOnInit() {
    ...
    Promise.all([
      this.http
        .get(
          `${this.appSettings.apiSAP}/api/batch/${this.productData.FromWhsCod}/${this.productData.ItemCode}`
        )
        .toPromise(),
      ...
      .then(([data]: any) => {
        if (data[0].BatchNum == null) {
          this.presentToast(
            "Producto Sin Lotes Disponibles. Notificar A Adutoria.",
            "danger"
          );
          return;
        }
  ...
  public addLote() {
    ...
    let unitBase = this.productData.Uoms.findIndex(
    ...
    if (dif < 2) {
      ...
      if (
        ...
        this.presentToast(
          "La Cantidad Ingresada Excede De La Cantidad Solicitada",
          "warning"
        );
        return;
      }
  ...
  public transferAbarrotes() {
    ...
    if (
      this.productData.UomEntry == this.productData.Uoms[madeByUom].BaseEntry
    ) {
      this.productData.count = this.loteAEnviar
        .map((lote: any) => lote.Quantity)
        .reduce((a, b) => a + b, 0);
    } else {
