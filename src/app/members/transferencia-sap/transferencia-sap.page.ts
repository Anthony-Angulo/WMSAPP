...
export class TransferenciaSapPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    if (productsScanned.count <= 0) {
      let ind = this.products.findIndex(
        (product) => product.ItemCode == productsScanned.ItemCode
      );
      this.products.splice(ind, 1);
    } else {
      let index = this.transferData.Lines.findIndex(
        (product) => product.ItemCode == productsScanned.ItemCode
      );
  ...
  async getOrden() {
    ...
    this.http
      .get(
        `${this.appSettings.apiSAP}/api/InventoryTransferRequest/DeliverySAPNew/${this.transferRequestDocNum}`
      )
      .toPromise()
      .then((transferData: any) => {
        ...
        //   return
        // } else {
        return this.transferData.Lines.filter(
          (x: any) => x.U_IL_TipPes != "F"
  ...
  public searchProductByCode() {
    let index = this.transferData.Lines.findIndex(
      (x) => x.ItemCode == this.search.toUpperCase()
    );

    if (index < 0) {
      this.presentToast(
        "Codigo De Producto No Encontrado En La Orden",
        "warning"
      );
      return;
    }
    ...
    if (this.transferData.Lines[index].LineStatus == "C") {
      this.presentToast(
        "Este Producto Ya Fue Transferido Completamente y Se Cerro En El Documento De Origen.",
        "warning"
      );
      return;
    }
  ...
  public searchProductByCb() {
    ...
    if (
      this.transferData.Lines[index].CodeBars[found].UomEntry !=
      this.transferData.Lines[index].UomEntry
    ) {
      this.presentToastmid(
        "La unidad de medida escaneada no es igual a la del pedido.",
        "warning"
      );
    }

    let total =
      Number(this.transferData.Lines[index].CodeBars[found].BaseQty) *
      Number(this.ctd);

    if (total > this.transferData.Lines[index].OpenInvQty) {
      this.presentToast(
        "La Cantidad Registrada Excede La Cantidad Requerida",
        "danger"
      );
      return;
    }
    ...
    if (this.transferData.Lines[index].LineStatus == "C") {
      this.presentToast(
        "Producto ya fue transferido completamente.",
        "warning"
      );
      this.ctd = null;
      this.tarima = "";
    ...
    if (this.transferData.Lines[index].ManBtchNum == "Y") {
      ...
      console.log(
        getValidPercentage(
          this.transferData.Lines[index],
          this.appSettings.porcentaje
        )
      );
      ...
      this.http
        .get(
          `${this.appSettings.apiSAP}/api/batch/${this.transferData.Lines[index].FromWhsCod}/${this.transferData.Lines[index].ItemCode}`
        )
        .toPromise()
        .then((r: any) => {
          if (r.stock <= 0) {
            this.presentToast(
              `Stock En Negativo Para el Producto: ${this.transferData.Lines[index].ItemCode}`,
              "warning"
            );
          } else if (this.ctd > r.stock) {
            this.presentToast(
              `Stock Insuficiente Para el Producto: ${this.transferData.Lines[index].ItemCode} Stock: ${r.stock}`,
              "warning"
            );
          } else {
        ...
        .catch((err) => {
          this.presentToast(
            `Error Al Buscar Stock Para El Producto ${this.transferData.Lines[index].ItemCode}`,
            "danger"
          );
        })
        .finally(() => {
  ...
  async sendProducts() {
    ...
    const TransferRows = this.transferData.Lines.filter(
    ...
    if (TransferRows.length != 0) {
      ...
      this.http
        .post(
          this.appSettings.apiSAP + "/api/inventorytransfer/SAP",
          recepcionData
        )
        .toPromise()
        .then((data: any) => {
