...
export class AbarrotesBatchPage implements OnInit {
  ...
  getGS1Data() {
    ...
    if (this.productData.Detail.PurPackUn == 1) {
      if (Number(this.cantidad) > 100) {
        this.presentToast(
          "La cantidad excede maximo de cajas por tarima",
          "warning"
        );
        this.codeBar = "";
    ...
    try {
      ...
      for (var x = 0; x < answer.parsedCodeItems.length; x++) {
        ...
        if (
          ...
          prodDetail.manufacturingDate = formatDate(
        ...
        if (answer.parsedCodeItems[x].ai == "12") {
          prodDetail.expirationDate = formatDate(
            answer.parsedCodeItems[x].data,
            "yyyy-MM-dd",
            "en-US"
          );
          ...
          prodDetail.expirationDate = new Date(
        ...
        if (
          ...
          answer.parsedCodeItems[x].ai == "3200"
        ) {
          prodDetail.quantity = Number(
            Number(
              Number(this.cantidad * Number(answer.parsedCodeItems[x].data)) /
                2.2046
            ).toFixedNoRounding(7)
          );
          this.peso = prodDetail.quantity;
          console.log(
            Number(Number(answer.parsedCodeItems[x].data)).toFixedNoRounding(4)
          );
        } else if (
          ...
          prodDetail.quantity = Number(
            Number(
              this.cantidad * Number(answer.parsedCodeItems[x].data)
            ).toFixedNoRounding(4)
          );
          this.peso = prodDetail.quantity;
        }
  ...
  async imprimirTarima(prodDetail: any) {
    ...
    this.http
      .get(
        ...
        }&Total=${Number(this.peso)}
    &UoM=${this.productData.UomEntry}&DocNum=${
          this.productData.DocNum
        }&Cajas=${Number(this.cantidad)}&printer=Label-Test23`
      )
      .toPromise()
      .then((x: any) => {
        console.log(x);
  ...
  captureData() {
    ...
    if (
      this.productData.UomEntry == "486" ||
      this.productData.UomEntry == "485"
    ) {
      this.lotes.push({
        name: "SI",
        expirationDate: "2024-01-01",
        quantity: Number("17418.240000"),
        code: "",
        att1: "",
        pedimento: "",
      });
