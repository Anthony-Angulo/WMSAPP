...
import {
  getSettingsFileData,
  getCerosFromEtiquetaInventario,
} from "../../commons";
...
export class BeefPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (
      Number(this.productData.Detail.U_IL_PesMin) == 0 &&
      Number(this.productData.Detail.U_IL_PesMax) == 0
    ) {
    ...
    await Promise.all([
      this.http
        ...
        .toPromise(),
      this.http
        .get(`${environment.apiCCFN}/codeBar/${this.productData.ItemCode}`)
        .toPromise(),
      this.http
        .get(`${environment.apiCCFN}/crBar/${this.productData.ItemCode}`)
        .toPromise(),
    ])
    ...
    if (this.productData.Detail.QryGroup45 == "Y") {
      this.codebarDescription = this.productData.cBDetail.filter(
        (x) => x.OriginLocation != null
      );
  ...
  submit() {
    ...
    if (this.productData.count != 0 && this.cantidadEscaneada <= 0) {
      ...
      this.productData.count = this.detail
        .map((lote) => lote.quantity)
        .reduce((a, b) => a + b, 0);

      let validPercent =
        (Number(this.porcentaje) / 100) * Number(this.productData.OpenInvQty);
      let validQuantity =
        Number(validPercent) + Number(this.productData.OpenInvQty);

      if (this.productData.count > Number(validQuantity)) {
        this.presentToast(
          "Cantidad Excede a la cantidad solicitada",
          "warning"
        );
      } else {
        // this.productData.count = this.cantidadEscaneada
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: (data) => {
            ...
            let isScanned = this.detail.findIndex(
              (prod) => prod.code == data[0]
            );
            ...
            if (
              this.navExtras.getPedimento() == undefined &&
              this.productData.QryGroup2 == "Y"
            ) {
  ...
  public getDataFromGS1() {
    ...
    if (this.codigoBarra.trim()[0] == "C") {
      let isScanned = this.productData.crBars.findIndex(
        (prod: any) => prod.CodeBar == this.codigoBarra.trim()
      );
      if (isScanned >= 0) {
    ...
    let isScanned = this.detail.findIndex(
      (prod) => prod.code == this.codigoBarra.trim()
    );
    let availableBatch = this.productData.batchs.findIndex(
      (prod) => prod.U_IL_CodBar == this.codigoBarra.trim()
    );
    ...
    try {
      ...
      for (var x = 0; x < answer.parsedCodeItems.length; x++) {
        ...
        if (
          answer.parsedCodeItems[x].ai == 13 ||
          answer.parsedCodeItems[x].ai == 11
        ) {
          this.detailGS1.manufacturingDate = formatDate(
            answer.parsedCodeItems[x].data,
            "yyyy-MM-dd",
            "en-US"
          );
          this.FechaProd = formatDate(
            answer.parsedCodeItems[x].data,
            "yyyy-MM-dd",
            "en-US"
          );
        ...
        if (answer.parsedCodeItems[x].ai == "12") {
          this.detailGS1.expirationDate = formatDate(
            answer.parsedCodeItems[x].data,
            "yyyy-MM-dd",
            "en-US"
          );
          this.FechaCad = formatDate(
            answer.parsedCodeItems[x].data,
            "yyyy-MM-dd",
            "en-US"
          );
        } else if (
          this.productData.Detail.U_IL_DiasCad > 0 &&
          answer.parsedCodeItems[x].ai == "11"
        ) {
          console.log(this.productData.Detail.U_IL_DiasCad);
          this.detailGS1.expirationDate = new Date(
            answer.parsedCodeItems[x].data.setDate(
              answer.parsedCodeItems[x].data.getDate() +
                Number(this.productData.Detail.U_IL_DiasCad)
            )
          ).toISOString();
          this.detailGS1.expirationDate = formatDate(
            this.detailGS1.expirationDate,
            "yyyy-MM-dd",
            "en-US"
          );
          this.FechaCad = formatDate(
        ...
        if (
          answer.parsedCodeItems[x].ai == "3201" ||
          answer.parsedCodeItems[x].ai == "3202"
        ) {
          this.detailGS1.quantity = Number(
            Number(answer.parsedCodeItems[x].data / 2.2046).toFixed(2)
          );
          if (
            this.detailGS1.quantity == 0 ||
            this.detailGS1.quantity == undefined
          ) {
            this.getDataFromCodeBar();
            return;
          }
          ...
          answer.parsedCodeItems[x].ai == "3101" ||
          answer.parsedCodeItems[x].ai == "3102"
        ) {
          this.detailGS1.quantity = Number(
            Number(answer.parsedCodeItems[x].data).toFixed(2)
          );
          if (
            this.detailGS1.quantity == 0 ||
            this.detailGS1.quantity == undefined
  ...
  public getDataFromCodeBar(): void {
    ...
    if (this.productData.Detail.QryGroup45 == "Y") {
      codeBarSettings = this.productData.cBDetail.findIndex(
        (y: any) => y.OriginLocation == this.selectedDesc
      );
      console.log(codeBarSettings);
    } else {
      codeBarSettings = this.productData.cBDetail.findIndex(
        (y: any) => y.BarcodeLength == this.codigoBarra.trim().length
      );
    }

    if (codeBarSettings < 0) {
      this.presentToast(
        "El Codigo De Barra No Coincide Con la Configuracion Del Codigo De Barra Del Producto.",
        "warning"
      );
      this.peso = 0;
      document.getElementById("input-codigo").setAttribute("value", "");
    ...
    let pesoDeEtiqueta = this.codigoBarra.substr(
      this.productData.cBDetail[codeBarSettings].WeightPosition - 1,
      this.productData.cBDetail[codeBarSettings].WeightLength
    );

    if (
      this.productData.cBDetail[codeBarSettings].HasDecimal.data[0] == 1 &&
      this.productData.cBDetail[codeBarSettings].UoM == 4
    ) {
      this.peso = Number((Number(pesoDeEtiqueta) / 2.2046).toFixed(2));
      console.log(this.peso);
    } else if (
      this.productData.cBDetail[codeBarSettings].HasDecimal.data[0] == 1 &&
      this.productData.cBDetail[codeBarSettings].UoM == 3
    ) {
      this.peso = Number(pesoDeEtiqueta);
    } else {
      this.peso = getCerosFromEtiquetaInventario(
        this.productData,
        pesoDeEtiqueta,
        codeBarSettings
      );
    ...
    if (
      this.peso < this.productData.U_IL_PesMin ||
      this.peso > this.productData.U_IL_PesMax
    ) {
      this.presentToast(
        "El Peso Escaneado No Esta Dentro De Los Parametros De Peso Maximo O Peso Minimo.",
        "warning"
      );
      this.peso = 0;
      document.getElementById("input-codigo").setAttribute("value", "");
  ...
  async imprimirTarima() {
    ...
    this.http
      .get(
        `${environment.apiSAP}/api/Impresion/PruebaReciboTarima?Itemcode=${
          this.productData.ItemCode
        }&Total=${Number(this.cantidadPeso)}
    &UoM=${this.productData.UomEntry}&DocNum=${
          this.productData.DocNum
        }&Cajas=${Number(this.cantidadEscaneada)}&printer=${
          this.appSettings.IpImpresora
        }`
      )
      .toPromise()
      .then((lo: any) => {
