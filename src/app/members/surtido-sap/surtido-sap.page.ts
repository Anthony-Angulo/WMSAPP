...
export class SurtidoSapPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    if (productsScanned.DeliveryRowDetailList) {
      let index = this.order.Lines.findIndex(
      ...
      if (this.order.Lines[index].QryGroup42 == "Y") {
        ...
        this.order.Lines[index].count =
          productsScanned.DeliveryRowDetailList.map(
            (prod) => prod.Cajas
          ).reduce((a, b) => a + b, 0);
      } else {
        console.log(this.order.Lines[index]);
      ...
      let isScanned = this.products.findIndex(
        (prd) => prd.ItemCode == productsScanned.ItemCode
      );
      if (isScanned < 0) {
  ...
  async getOrden() {
    ...
    this.http
      .get(`${this.appSettings.apiSAP}/api/order/DeliverySAPNew/${this.number}`)
      .toPromise()
      .then((data: any) => {
        this.order = data;
      })
  ...
  public searchProductByCb() {
    ...
    if (
      this.order.Lines[index].CodeBars[found].UomEntry !=
      this.order.Lines[index].UomEntry
    ) {
  ...
  async sendProducts() {
    ...
    const DeliveryRowDetailList = this.products.map((product) => {
      return {
        ...
        DeliveryRowDetailList: product.DeliveryRowDetailList
          ? product.DeliveryRowDetailList
          : [],
      };
    });
    ...
    if (DeliveryRowDetailList.length != 0) {
      ...
      this.http
        .post(`${this.appSettings.apiSAP}/api/Delivery/SAP`, recepcionData)
        .toPromise()
        .then((data: any) => {
          console.log(data);
          this.sendUpdateCr();
  ...
  async sendUpdateCr() {
    await this.http
      .put(`${environment.apiCCFN}/crbar/updateCr`, this.crBars)
      .toPromise();
  }
