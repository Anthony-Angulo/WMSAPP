...
export class BeefPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (
      Number(this.productData.U_IL_PesMin) == 0 &&
      Number(this.productData.U_IL_PesMax) == 0
    ) {
  ...
  public getCodeBarCR(): void {
    let madeByUom = this.productData.Uoms.findIndex(
    ...
    let alreadyScanned = this.detail.findIndex(
      (y: any) => y.CodeBar == this.codigoBarra.trim()
    );
    ...
    let foundInBatch = this.batch.findIndex(
      (code: any) => code.U_IL_CodBar == this.codigoBarra.trim()
    );

    if (foundInBatch < 0) {
      let loteGenerico = this.batch.find(
        (y: any) => y.BatchNum.toUpperCase() == "SI"
      );
  ...
  public getDataFromCodeBar(): void {
    ...
    if (this.productData.QryGroup45 == "Y") {
      codeBarSettingInd = this.productData.cBDetail.findIndex(
    ...
    let pesoDeEtiqueta = this.codigoBarra.substr(
      this.productData.cBDetail[codeBarSettingInd].WeightPosition - 1,
      this.productData.cBDetail[codeBarSettingInd].WeightLength
    );

    if (
      this.productData.cBDetail[codeBarSettingInd].HasDecimal.data[0] == 1 &&
      this.productData.cBDetail[codeBarSettingInd].UoM == 4
    ) {
      this.peso = Number((Number(pesoDeEtiqueta) / 2.2046).toFixed(2));
    } else if (
      this.productData.cBDetail[codeBarSettingInd].HasDecimal.data[0] == 1 &&
      this.productData.cBDetail[codeBarSettingInd].UoM == 3
    ) {
    ...
    let isCodeBarExist = this.batch.findIndex(
      (codeBar: any) => codeBar.U_IL_CodBar == this.codigoBarra.trim()
    );
    ...
    if (isCodeBarExist >= 0) {
      ...
      if (loteGenerico != undefined) {
        ...
        this.presentToast(
          "El Codigo de Barra No Fue Encontrado En Inventario O No Hay Cantidad Suficiente En Lote Generico. Revisar Con Auditoria.",
          "warning"
        );
      }
    }
