...
@Component({
  ...
  templateUrl: './inventario-ciclico.page.html',
  styleUrls: ['./inventario-ciclico.page.scss'],
})

export class InventarioCiclicoPage implements OnInit {
  ...
  goToOrderDetail(index: number) {
    let inventory_detail = this.inventory_orders.detalle
      .filter(x => x.MovimientoInventario_id == this.inventory_orders.orders[index].id)
    this.navExtras.setInventoryDetail(inventory_detail)
